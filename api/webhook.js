export default async function handler(req, res) {
  ...
  try {
    ...
    const nome = data?.Customer?.name || data?.customer?.name ||
                 data?.buyer?.name || email.split('@')[0] || 'aluno';
    ...
    // Atualizar ou criar usuario no Supabase
    ...
    if (
      resendKey &&
      (evento === 'order_approved' || evento === 'subscription_renewed')
    ) {
  ...
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;margin-top:20px;">
    ...
    <div style="padding:32px 24px;">
      ...
      <div style="background:#FFF4EE;border-left:4px solid #FF6B00;border-radius:8px;padding:16px;margin:24px 0;">
        ...
        <p style="margin:0;color:#444;font-size:14px;">
          Instale o app no seu celular para ter acesso mais rápido e não perder seu progresso.
        </p>
      </div>
    ...
    <div style="background:#f5f5f5;padding:16px 24px;text-align:center;">
      ...
      try {
        ...
        // Nao falha o webhook por causa do email
      }
    }
